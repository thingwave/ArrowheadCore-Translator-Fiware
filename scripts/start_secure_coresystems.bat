...
START "" /B "cmd /c javaw -jar -DarrowheadSystem=authorization arrowhead-authorization-4.0.jar -d -daemon -tls > secure_auth.log 2>&1"
echo Authorization started

cd ..\..\gateway\target
START "" /B "cmd /c javaw -jar -DarrowheadSystem=gateway arrowhead-gateway-4.0.jar -d -daemon -tls > secure_gateway.log 2>&1"
echo Gateway started

cd ..\..\eventhandler\target
START "" /B "cmd /c javaw -jar -DarrowheadSystem=eventhandler arrowhead-eventhandler-4.0.jar -d -daemon -tls > secure_eventhandler.log 2>&1"
echo Event Handler started

cd ..\..\gatekeeper\target
START "" /B "cmd /c javaw -jar -DarrowheadSystem=gatekeeper arrowhead-gatekeeper-4.0.jar -d -daemon -tls > secure_gk.log 2>&1"
echo Gatekeeper started

cd ..\..\orchestrator\target
START "" /B "cmd /c javaw -jar -DarrowheadSystem=orchestrator arrowhead-orchestrator-4.0.jar -d -daemon -tls > secure_orch.log 2>&1"
